...
# ------------------------------
# Initialize lists to store epoch-wise loss values
epoch_loss_Bx = []
epoch_loss_By = []
epoch_loss_Bz = []
# ------------------------------
...
for n_ep in np.arange(N_epochs):
    ...
    start_idx = n_ep * Nt
    end_idx = (n_ep + 1) * Nt
    epoch_loss_Bx.append(np.mean(hist_Bx[start_idx:end_idx]))
    epoch_loss_By.append(np.mean(hist_By[start_idx:end_idx]))
    epoch_loss_Bz.append(np.mean(hist_Bz[start_idx:end_idx]))
    # ------------------------------
...
plt.plot(epochs, epoch_loss_Bx, label='Loss Bx', marker='o')
plt.plot(epochs, epoch_loss_By, label='Loss By', marker='o')
plt.plot(epochs, epoch_loss_Bz, label='Loss Bz', marker='o')

plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.title('Loss Bx, By, Bz as a function of Epochs')
plt.legend()
plt.grid(True)
plt.show()
# ------------------------------